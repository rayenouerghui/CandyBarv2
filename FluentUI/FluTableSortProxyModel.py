from PySide6.QtCore import Signal, Property, QSortFilterProxyModel, Slot, Qt, QObject
...
class FluTableSortProxyModel(QSortFilterProxyModel):
    ...
    def getRow(self, rowIndex: int):
        source_row = self.mapToSource(self.index(rowIndex, 0)).row()
        return self._model.getRow(source_row)

    # noinspection PyTypeChecker
    @Slot(int, dict)
    def setRow(self, rowIndex: int, val):
        source_row = self.mapToSource(self.index(rowIndex, 0)).row()
        self._model.setRow(source_row, val)

    @Slot(int, int)
    def removeRow(self, rowIndex: int, rows: int):
        source_row = self.mapToSource(self.index(rowIndex, 0)).row()
        self._model.removeRow(source_row, rows)
    ...
    def lessThan(self, source_left, source_right):
        if self._comparator is None or self._comparator.isUndefined():
            return True
        data = [source_left.row(), source_right.row()]
