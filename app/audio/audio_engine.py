...
class AudioEngine(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # Set when audio asset copying to the writable runtime directory has completed
        self.copy_completed = threading.Event()
    ...
    def _worker_loop(self):
        ...
        while True:
            ...
            # Playback cannot start before the background audio-copy operation has finished
            self.copy_completed.wait()
        ...
        try:
            ...
            for rel in files:
                ...
                if not os.path.isfile(full):
                    ...
                    continue
                # Verify that the file size is non-zero (not corrupt/truncated)
                try:
                    if os.path.getsize(full) == 0:
                        logger.error(f"Cannot play {rel}: file size is 0 bytes (corrupt/truncated)")
                        continue
                except OSError as e:
                    logger.error(f"Could not verify file size for {full}: {e}")
                    continue
                try:
    ...
    def _build_sequence(self, number: str) -> List[str]:
        """Build the ordered list of MP3 relative paths to play: Chime -> Number word -> Number."""
        ...
        # 2. Then the spoken word "number" (English/French/Arabic)
