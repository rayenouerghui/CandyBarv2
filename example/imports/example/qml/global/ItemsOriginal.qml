...
FluObject{
    ...
    FluPaneItem{
        ...
        menuDelegate: paneItemMenu
        url: "qrc:/example/qml/page/T_Display.qml"
        onTap: { navigationView.push(url) }
    }
    ...
    FluPaneItemExpander{
        ...
        FluPaneItem{
            ...
            title: "Order Status"
            menuDelegate: paneItemMenu
            infoBadge: FluBadge{ id: badge_orders; count: 0 }
            url: "qrc:/example/qml/page/T_OrderStatus.qml"
            onTap: { badge_orders.count = 0; navigationView.push(url) }
        }

        FluPaneItem{
            title: "MQTT Test"
            menuDelegate: paneItemMenu
            url: "qrc:/example/qml/page/T_MqttTest.qml"
            onTap: { navigationView.push(url) }
        }
        ...
        FluPaneItem{
            ...
            title: "Message Log"
            menuDelegate: paneItemMenu
            infoBadge: FluBadge{ id: badge_log; count: 0 }
            url: "qrc:/example/qml/page/T_MessageLog.qml"
            onTap: { badge_log.count = 0; navigationView.push(url) }
        }
    }

    Connections {
        target: MqttClient
        function onOrderReceived(orderJson)      { badge_orders.count++ }
        function onMessageReceived(topic, payload) { badge_log.count++ }
    }
