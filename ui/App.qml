...
Window {
    ...
    // Bind to actual screen dimensions for embedded platforms (Raspberry Pi / eglfs)
    width: Screen.width
    height: Screen.height

    // Set minimum size to prevent window from shrinking too small
    minimumWidth: 1280
    minimumHeight: 720
    ...
    Item {
        ...
        Keys.onPressed: function(event) {
            if (event.key === Qt.Key_Escape) {
                // Set window size before showing normal to avoid tiny window in VM
                root.showNormal()
                // Use a timer to ensure size is set after window is shown
